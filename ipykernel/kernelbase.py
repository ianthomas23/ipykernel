...
class Kernel(SingletonConfigurable):
    ...
    def _shell_thread_worker(self, shell_id, shell_id_dict, acontext):
        ...
        socket = acontext.socket(zmq.ROUTER)  # acontext or context ???
        socket.linger = 1000
        ...
        # Maybe want to store socket, stream, io_loop, etc
    ...
    async def create_subshell_request(self, stream, ident, parent):
        shell_id = str(uuid.uuid4())  # Need some sort of unique identifier, socket_port might do?

        shell_id_dict = {"port": 0}
        # Create and start subshell thread.
        self._create_shell_thread(shell_id, shell_id_dict)

        # Should wait until port is available here...
        await asyncio.sleep(0.1)
        port = shell_id_dict["port"]

        self.subshell_cache[shell_id] = shell_id_dict

        connection_info = self.parent.get_connection_info()
        connection_info["shell_port"] = port

        content = {
            "status": "ok",
            "shell_id": shell_id,  # May not be needed?
